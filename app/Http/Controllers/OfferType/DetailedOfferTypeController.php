...
/**
 * DetailedOfferTypeController.php
 *
 * Handles detailed offer type module's functionalities and related operations.
 *
 * @access    public
...
class DetailedOfferTypeController extends MyController
{
  /*
  |--------------------------------------------------------------------------
  | Detailed Offer Type Controller
  |--------------------------------------------------------------------------
  |
  | This controller handles all the crud functionalities related to detailed
  | offer type module. It includes project and detailed offer type
  ...
  */
  ...
  public function postEdit(Request $request){
    try {
      ...
      $offer_type_details = $this->detailed_offer_type_model->getDetailedOfferTypeById($request->input('post_id'));
      $offer_type_details[1] = ['type' => 'detailed'];
  ...
  /**
   * Get detailed offer type id from incoming request and delete the same record.
   * @param  \Illuminate\Http\Request $request
   * @return mixed
   */
  public function deleteDelete(Request $request){
    try {
      ...
      //Delete requested detailed offer type from database, throw exception if requested
      //offer type couldnot delete else return offer types listing page
      //with success notice.
      $delete_flag = $this->detailed_offer_type_model->deleteDetailedOfferType($delete_id);
      if($delete_flag == false):
        throw new Exception('Could not delete requested offer type. Please try again.');
      endif;
...
}//End Class DetailedOfferTypeController
